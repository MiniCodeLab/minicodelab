...
Para el tipado de Ojectos tenemos la palabra **`object`** , esto nos permite tipar nuestros objetos como object. Aunque no es buena práctica porque es excesivamente laxo, es decir acepta como válido cualquier tipo de objeto.
...
En la primera función no retornamos ningún tipo de valor y por lo tanto podemos indicar que no tiene retorno, es decir **`void`**. Si nosootros intentasemos retornar un valor Typescript se quejará.
...
Imagina que necesitamos pasar el valor obtenido por la función del ejemplo anterior a otra función, como hemos con `parseInt`, pero falla al ser de tipo `unknown`. Esto podemos solucionarlo si tenemos la seguridad de que esta función de volverá un string, realizando `Type Assertion` con la palabra `as`:
