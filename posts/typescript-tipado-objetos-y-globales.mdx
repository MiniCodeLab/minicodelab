...
Con esto estamos diciendo que `person` puede ser un objeto que tenga claves de tipo `string` y valores de tipo `string` o `number` para dichas claves. Podemos observar con esto que crearemos tipos bastante dinámicos, pero con el riesgo de confundir claves, por lo que hay que usar `Record` con cierta precaución.
...
Para solucionar esto, podemos tipar `networks` de forma más abstracta con `Record<string, string>` y solucionar este problema:
